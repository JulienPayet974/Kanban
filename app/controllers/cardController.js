  ...
  async update (req, res){
    try{
      ...
      if (typeof(req.body.content) !== "undefined" && typeof(req.body.content) !== "string"){
        return res.status(400).json({ error: "Invalid body parameter 'content'. Should provide a string." });
      }
      ...
      if ((!req.body.list_id && !req.body.content && !req.body.position && !req.body.color) && req.body.position !== 0){
        return res.status(400).json({ "error": "Invalid body. Should provide at least a 'content', 'color' or 'position' property" });
      }
      ...
      if (req.body.content){
        cardNewData.content = req.body.content;
      } 
